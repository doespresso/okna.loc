...
class sbDbCache implements sbICache
{
    ...
    /**
     * Список компонентов, которые НЕ должны кешироваться
     * @var array
     */
    private $mComponentNoCache = array();
    ...
    public function check($plugin_ident, $tag_id, $params, $use_session=true)
    {
        ...
        {
            $this->mComponentNoCache[] = $this->mHash;
            return false;
        }
    ...
    public function save($plugin_ident, $str, $el_id = '')
    {
    	...
    	$str = preg_replace('/(.*?)\{\s*?\$\s*?\{.*?\}\s*?\}(.*?)/imsu', '$1$2', $str);
        $str = preg_replace('/(.*?)\$\s*?\{.*?\}(.*?)/imsu', '$1$2', $str);
        ...
        if(($key = array_search($this->mHash, $this->mComponentNoCache)) !== false)
        {
            unset($this->mComponentNoCache[$key]);
            $this->mHash = array_pop($this->mHashAr);
            return;
        }
