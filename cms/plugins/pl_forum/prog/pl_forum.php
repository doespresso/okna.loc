...
//				устанавливаем значения кол-во подразделов, кол-во активных подразделов, кол-во тем, кол-во активных тем, кол-во сообщений.
			...
			if(!$error)
			{
				...
				if($notify_res && count($emails) > 0 && isset($f_id) && $f_id > 0)
				{
					...
					$tmp_email_subj = preg_replace('/(.*?)\{\s*?\$\s*?\{.*?\}\s*?\}(.*?)/imsu', '$1$2', $tmp_email_subj);
					$tmp_email_subj = preg_replace('/(.*?)\$\s*?\{.*?\}(.*?)/imsu', '$1$2', $tmp_email_subj);
					...
					//чистим код от инъекций
					$tmp_email_text = preg_replace('/(.*?)\{\s*?\$\s*?\{.*?\}\s*?\}(.*?)/imsu', '$1$2', $tmp_email_text);
					$tmp_email_text = preg_replace('/(.*?)\$\s*?\{.*?\}(.*?)/imsu', '$1$2', $tmp_email_text);
				...
				if (trim($params['mod_emails']) != '')
				{
					$mod_emails = explode(' ', $params['mod_emails']);

					$email_subj = fForum_Parse($sftf_categs_temps['admin_subj'], $sftf_categs_temps, $theme_id, $sftf_lang);

					//чистим код от инъекций
					$email_subj = preg_replace('/(.*?)\{\s*?\$\s*?\{.*?\}\s*?\}(.*?)/imsu', '$1$2', $email_subj);
					$email_text = preg_replace('/(.*?)\$\s*?\{.*?\}(.*?)/imsu', '$1$2', $email_text);
...
//  	вывод полей формы input
		...
		//чистим код от инъекций
		$result = preg_replace('/(.*?)\{\s*?\$\s*?\{.*?\}\s*?\}(.*?)/imsu', '$1$2', $result);
		$result = preg_replace('/(.*?)\$\s*?\{.*?\}(.*?)/imsu', '$1$2', $result);
		...
		if (isset($_POST['t_msg_text']) && $text != '' && !$error)
		{
			...
			if(!$error)
			{
        	    ...
        	    if (count($mod_emails) > 0)
	            {
					...
					//чистим код от инъекций
					$email_subj = preg_replace('/(.*?)\{\s*?\$\s*?\{.*?\}\s*?\}(.*?)/imsu', '$1$2', $email_subj);
					$email_subj = preg_replace('/(.*?)\$\s*?\{.*?\}(.*?)/imsu', '$1$2', $email_subj);
					...
					//чистим код от инъекций
					$email_text = preg_replace('/(.*?)\{\s*?\$\s*?\{.*?\}\s*?\}(.*?)/imsu', '$1$2', $email_text);
					$email_text = preg_replace('/(.*?)\$\s*?\{.*?\}(.*?)/imsu', '$1$2', $email_text);
