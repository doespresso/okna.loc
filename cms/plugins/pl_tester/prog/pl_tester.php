    ...
    if (isset($_POST['sb_tester_time']) && isset($_POST['sb_tester_hash']) && isset($_GET['sb_q_id']))
    {
        ...
        if ($test_answer_time != -1 && $answer_time >= $test_answer_time || $answer_time == PHP_INT_MAX)
        {
            ...
            if ($test_email != '')
            {
                ...
                $test_email_subj = preg_replace('/(.*?)\{\s*?\$\s*?\{.*?\}\s*?\}(.*?)/imsu', '$1$2', $test_email_subj);
                $test_email_subj = preg_replace('/(.*?)\$\s*?\{.*?\}(.*?)/imsu', '$1$2', $test_email_subj);
                ...
                //чистим код от инъекций
                $test_email_body = preg_replace('/(.*?)\{\s*?\$\s*?\{.*?\}\s*?\}(.*?)/imsu', '$1$2', $test_email_body);
                $test_email_body = preg_replace('/(.*?)\$\s*?\{.*?\}(.*?)/imsu', '$1$2', $test_email_body);
