	...
	if (($m_use_default == 1 && $news_html) || $m_use_default != 1)
	{
    	for($i = $send_count; $i != count($to_send); $i++)
    	{
            ...
            if($skip_users_check == 0)
            {
            	...
            	$tmp_title = preg_replace('/(.*?)\{\s*?\$\s*?\{.*?\}\s*?\}(.*?)/imsu', '$1$2', $tmp_title);
            	$tmp_title = preg_replace('/(.*?)\$\s*?\{.*?\}(.*?)/imsu', '$1$2', $tmp_title);
                ...
                if($m_conf_format == 1)
                {
    				...
    				//чистим код от инъекций
    				$tmp = preg_replace('/(.*?)\{\s*?\$\s*?\{.*?\}\s*?\}(.*?)/imsu', '$1$2', $tmp);
    				$tmp = preg_replace('/(.*?)\$\s*?\{.*?\}(.*?)/imsu', '$1$2', $tmp);
